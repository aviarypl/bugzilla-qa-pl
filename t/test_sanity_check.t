...
go_to_admin($sel);
$sel->click_ok("link=Spójność danych", undef, "Go to Sanity Check (no parameter)");
$sel->wait_for_page_to_load(WAIT_TIME);
$sel->title_is("Sprawdzanie spójności danych", "Display sanitycheck.cgi");
$sel->is_text_present_ok("Test spójności danych został zakończony.", undef, "Page displayed correctly");
...
foreach my $arg (@args) {
    ...
    $sel->title_is("Podejrzana czynność", "Calling sanitycheck.cgi with no token triggers a confirmation page");
    $sel->click_ok("confirm", "Confirm the action");
    $sel->wait_for_page_to_load(WAIT_TIME);
    $sel->title_is("Sprawdzanie spójności danych", "Calling sanitycheck.cgi with $arg=1");
    if ($arg eq 'rescanallBugMail') {
        ...
        $sel->is_text_present_ok("niewysłanych wiadomości", undef, "Look for unsent bugmail");
